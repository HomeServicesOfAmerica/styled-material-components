  ...
  <ThemeProvider>
    <div>
      ...
      <h4>
        ...
        <ul>
          <li>header?: string The table header</li>
          <li>
            {`fields: Array of fields, which correspond to the columns. Each field
            should have the following properties:`}
            <ul>
              <li>key: string, // React key, must be unique</li>
              <li>label: string,</li>
              <li>numerical?: boolean</li>
              <li>sortable?: boolean</li>
            </ul>
          </li>
          <li>
          ...
          <li>
            {`rowsPerPage?: number This will default to 10. Default behavior if
            that pagination will appear if the number of rows exceeds the
            rowsPerPage`}
          </li>
          <li>
            {`currentPage?: number This will default to 1. You can use this to
            override which page you're on (if pagination is showing)`}
          </li>
          <li>
            {`totalDataPoints?: number Use this if you're "faking" pagination by
            passing in new data for each page, for example by fetching new data
            on paginate`}
          </li>
          <br />
          <li>Controlled Data:</li>
          <li>
            {`rowsDropDown?: [int] array of ints that specifies how many rows the
            user can choose from defaults to Fibonacci * rowsPerPage supplying
            an empty array will hide the "rowsPerPage"`}
          </li>
          <li>
            {`
              handleForwardPagination: () => any Callback that will be called when
              the user clicks the 'next' arrow
            `}
          </li>
          <li>
            {`handleBackwardsPagination: () => any Callback that will be called
            when the user clicks the 'back' arrow`}
          </li>
          <br />
          <li>
            {`onSort?: (key) => void Supply your own sorting method! Gives you
            'key' param! Remember to control your data!`}
          </li>
          <ul>
            <li>
              {`sortedBy?: string Current key that we are sorted by (used for row
              hilighting)`}
            </li>
            <li>
              {`descending?: boolean whether we are currently sorting by ascending
              or descending!`}
            </li>
          </ul>
          <li>notes on sorting:</li>
          <ul>
            <li>
              {`if Uncontrolled, the sort will order all the data regardles of the
              currentPage this is because it manipulates all the data it was
              given. Page agnostic.`}
            </li>
            <li>
              {`if controlled via a parent, it will manipulate only the data on
            ...
            <li>
              {`you can supply your own if you intend to mutate the data yourself
              see onSort`}
            </li>
          </ul>
          <br />
          <li>
            {`onSearch?: (string) => any function to be called onChange in the
            search input field. gives you the search input value`}
          </li>
          <br />
          <li>
            {`hasCheckboxes?: boolean denotes checkboxes on items, if this prop is
            passed you must also pass the next two methods below, for onCheck
            and onUncheck`}
          </li>
          <ul>
            <li>
              {`onCheck: () => any Callback that will be called when the user
              clicks a checkbox on. it is passed the Datum of that row as its
              first param`}
            </li>
            <li>
              {`onUncheck: () => any Callback that will be called when the user
              clicks a checkbox off. it is passed the Datum of that row as its
              first param`}
            </li>
          </ul>
        </ul>
      ...
      <h2>Regular Table</h2>
      <Table fields={fields} data={data} header="Table header" />
      <h2>Headless Table! :spooky:</h2>
      <Table fields={fields} data={data} />
      <h2>Table with column style override</h2>
      <StyledTable fields={fields} data={data} header="Table header" />
